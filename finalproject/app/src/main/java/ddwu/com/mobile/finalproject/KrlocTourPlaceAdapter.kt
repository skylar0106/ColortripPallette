...
class KrlocTourPlaceAdapter (var items : ArrayList<Place>) : RecyclerView.Adapter<KrlocTourPlaceAdapter.ViewHolder>() {
    // 뷰 홀더 만들어서 반환, 뷰릐 레이아웃은 list_item_tour.xml
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): KrlocTourPlaceAdapter.ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.krloc_tour_place_list, parent, false)
        return ViewHolder(itemView)
    }

    // 전달받은 위치의 아이템 연결
    override fun onBindViewHolder(holder: KrlocTourPlaceAdapter.ViewHolder, position: Int) {
